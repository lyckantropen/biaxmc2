...
std::vector<PRE79MeanProperties> do_recalculate(boostbase::base & db, const boostbase::tween_t_proxy & betweens, const boostbase::pair_t_proxy & wheres, const bool & commit = false, const std::string & commit_file = "")
{
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        PRE79StandardProperties * wwg2i = new PRE79StandardProperties(wwg2[i]);
        StandardL2Hamiltonian * H = new StandardL2Hamiltonian(t, l, tau, h);
...
void    table_output(const std::string & data_type, const std::vector<std::string> & columns, boostbase::base & db, const boostbase::tween_t_proxy & betweens, const boostbase::pair_t_proxy & wheres, bool recalculate = false)
{
    ...
    if(data_type == "thermalization_history")
    {
        ...
        std::cout << std::endl;
        for(const PRE79StandardProperties & prop : whatwegot)
        {
    ...
    if(data_type == "final_properties" || data_type == "properties")
    {
        ...
        */
        ...
        int count = 0;
        for(PRE79MeanProperties & prop : whatwegot)
        {
