...
{
    const Settings *  settings;         ///<a local reference to the global settings object
...
public:
    ...
    void RunParallel()
    {
        ...
        for(int i = 0; i < nscans; i++)         ///run over all values of the parameters
        {
            ...
            #pragma omp critical
            Log() << thread_sim.GetInternalLog();
            ///set the output to standard output
            thread_sim.SetStream(this);
            ///run the simulation
            thread_sim.Run();
    ...
    void RunParallelParallel()
    {
        ...
        for(int i = 0; i < nscans; i += incr)
        {
            ...
            for(int t = 0; t < current_nscans; t++)
            {
                ...
                #pragma omp critical
                Log() << termo.GetInternalLog();
                ///set the output to the main log
                termo.SetStream(this);
            ...
            for(int t = 0; t < current_nscans; t++)
            {
                ...
                ///flush the initialization log to the main log
                Log() << production.GetInternalLog();
                ///redirect the log to the main log
                production.SetStream(this);
